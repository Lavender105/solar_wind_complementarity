...
if __name__ == "__main__":
    ...
    ax_n.add_geometries(county_data["geometry"], crs=ccrs.PlateCarree(), fc="None", ec="black", linewidth=.03)
    ...
    sm = ScalarMappable(cmap=ListedColormap(['darkred']))
    ...
    cb.ax.text(1.2, 0.5, 'PV installations', va='center', ha='left', fontsize=14, fontproperties=prop, transform=cb.ax.transAxes)
    cb.outline.set_linewidth(0.5)
    cb.ax.tick_params(length=0)
    cb.set_ticklabels([])
    print(f'complete drawing colorbar, spend {t12 - t11} seconds.')

    save_path = os.path.join(output_dir, f'solar_location_power_map.png')
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    t13 = time.time()
    print(f'complete! save map to {save_path}. spend {t13 - t12} seconds.') # 2629s = 43min
